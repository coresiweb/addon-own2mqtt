class OpenWebNet:
    def __init__(self, options):
        self.own_server_address = (
            options['own_server_ip'], options['own_server_port'])
        self.own_password = options['own_server_password']

    def startSession(self, type_of):
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        logging.info('Starting %s session with %s',
                     type_of, self.own_server_address)
        ...
        if data_received == ACK:
            ...
            if data_received == b'*98*2##':
                ...
                logging.info('Connecting to MQTT Server %s:%s',
                             self.mqtt_server_ip, self.mqtt_server_port)

                mqtt_client = mqtt.Client()
                mqtt_client.username_pw_set(
                    self.mqtt_server_user, self.mqtt_server_password)
                ...
                while True:
                    ...
                    if data_received == b'':
                        ...
                        mqtt_client.publish(
                            'openwebnet/last_frame', payload=frame.frame, qos=0, retain=False)
